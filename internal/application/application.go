...
func CalcHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method != "POST" {
		w.WriteHeader(405)
		json.NewEncoder(w).Encode(Response{Error: fmt.Sprintf("%s", ErrInvalidMethod)})
		return
	}
	var request Request
	...
	if err != nil {
		w.WriteHeader(422)
		json.NewEncoder(w).Encode(Response{Error: fmt.Sprintf("%s", calculation.ErrUnexpectedEnd)})
		return
	}
	...
	if errors.Is(err, calculation.ErrDivisionByZero) || errors.Is(err, calculation.ErrUnexpectedEnd) || errors.Is(err, calculation.ErrAmountBrackets) || errors.Is(err, calculation.ErrNumberSearch) || errors.Is(err, calculation.ErrInvalidCharacter) {
		w.WriteHeader(422)
		json.NewEncoder(w).Encode(Response{Error: fmt.Sprintf("%s", err)})
		return
	} else if err != nil {
		w.WriteHeader(500)
		json.NewEncoder(w).Encode(Response{Error: fmt.Sprintf("%s", ErrInternalServer)})
		return
	}
